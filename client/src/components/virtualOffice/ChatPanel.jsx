    ...
    return (
        <div className="flex flex-col h-full bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl overflow-hidden">
            ...
            <div className="flex items-center gap-3 px-4 py-3 border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50">
                ...
                <span className="ml-auto text-xs bg-primary-theme/10 text-primary-theme px-2 py-0.5 rounded-full font-bold">{messages.length}</span>
            </div>
                ...
                {messages.map((msg) => {
                    ...
                    return (
                        <div key={msg.id} className={`flex gap-2 ${isMe ? 'flex-row-reverse' : 'flex-row'}`}>
                            <div className={`w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 ${msg.sender_role === 'admin' ? 'bg-primary-theme/10' : 'bg-emerald-100 dark:bg-emerald-900/30'}`}>
                            ...
                            <div className={`max-w-[75%] ${isMe ? 'items-end' : 'items-start'} flex flex-col gap-1`}>
                                ...
                                <div className={`px-3 py-2 rounded-2xl text-sm leading-relaxed break-words ${isMe
                                    ? 'bg-primary-theme text-white rounded-tr-sm'
            ...
            <form onSubmit={sendMessage} className="flex gap-2 p-3 border-t border-slate-100 dark:border-slate-800">
                <input
                    ...
                    className="flex-1 text-sm px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-primary-theme/30"
                    disabled={sending}
                />
                <button type="submit" disabled={sending || !input.trim()}
                    className="w-9 h-9 bg-primary-theme text-white rounded-xl flex items-center justify-center hover:opacity-90 disabled:opacity-40 transition-colors flex-shrink-0">
